...
const GEMINI_API_KEY = "AIzaSyDalvDi45ro8jnwXFl263ZD92tnWPnxrRU"; // Google Generative AI API Key shantanubombatkar5
...
export const askDetailedQuestions = async (
  initialResponses: ProjectResponses
  ...
  const detailedResponses = await prompt([
    {
      ...
      name: "purpose",
      message:
        "Please describe the main purpose and goals of your application:",
    },
    {
      type: "input",
      name: "features",
      message:
        "What are the key features you'd like to implement? (comma-separated)",
    },
  ]);
    ...
    return [
      {
        category: "General",
        question:
          "What additional details would you like to share about your project?",
      },
    ];
  }
};
...
${geminiAnalysis}
  ...
  const { proceed }: ProceedResponse = await prompt({
    ...
    name: "proceed",
    message:
      "Are you satisfied with these specifications and ready to proceed?",
    initial: true,
  });
...
export const runProjectSetup = async (): Promise<any> => {
  try {
    ...
    const { proceed, responses, analysis } = await confirmProject(
      fullResponses,
      finalAnalysis
    );
    ...
    if (proceed) {
      ...
      const { action }: any = await prompt({
        ...
        choices: [
          "Start over with new specifications",
          "Continue refining current specifications",
          "Exit setup",
        ],
      });
